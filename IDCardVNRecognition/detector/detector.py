...
class Detector:
    ...
    def process(self, aligned_image):
        ...
        if tmp is not None and 6 in list(info.keys()):
            if 'place_of_birth' in tmp.keys():
                info[9] = tmp['place_of_birth']
            if 'place_of_residence' in tmp.keys():
                info[10] = tmp['place_of_residence']
            info.pop(6)
