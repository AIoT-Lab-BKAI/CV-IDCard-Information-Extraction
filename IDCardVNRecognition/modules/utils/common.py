...
class poly():
    def __init__(self, segment_pts, type=1, value=''):  # segment_pts: [x1, y1, x2, y2, x3, y3, x4, y4]
        ...
        self.value = value
        # print(segment_pts)
        num_pts = int(len(segment_pts) / 2) # 4 pts
        # print('num_pts', num_pts)
        first_pts = [segment_pts[0], segment_pts[1]]    # return topleft: (x1, y1)
        self.list_pts = [first_pts] # [first_point]
        for i in range(1, num_pts):
            self.list_pts.append([segment_pts[2 * i], segment_pts[2 * i + 1]])
        # print('SELF list_pts', self.list_pts)        # return [p1, p2, p3, p4], pi = [xi, yi]
        # quit(0)
    ...
    def check_max_wh_ratio(self):
        max_ratio = 0
        if len(self.list_pts) == 4:     # 4 points
            first_edge = euclidean_distance(self.list_pts[0], self.list_pts[1]) # return width
            second_edge = euclidean_distance(self.list_pts[1], self.list_pts[2])    # return height
            if first_edge / second_edge > 1:    # return w/h
                long_edge = (self.list_pts[0][0] - self.list_pts[1][0], self.list_pts[0][1] - self.list_pts[1][1])  # return (x1 - x2, y1 - y2)
            else:
                long_edge = (self.list_pts[1][0] - self.list_pts[2][0], self.list_pts[1][1] - self.list_pts[2][1])  # return (x2 - x3, y2 - y3)
    ...
    def get_horizontal_angle(self):
        ...
        max_ratio, long_edge = self.check_max_wh_ratio()    # return (max(x1_x2/x2_x3, x2_x3/x1_x2) = max(w/h), (xi-xi+1, yi-yi+1) (i depends on w > h or not)
        # print('List points: ', self.list_pts)
        # print('Max_ratio: ', max_ratio)
        # print('Long edge: ', long_edge)
        if long_edge[0] == 0:   # only if x2-x3 <=> long_edge = (x2-x3, y2-y3)
            if long_edge[1] < 0:    # guess: vertical rect has p1, p2, p3,p4 (<- y2<y3 and x2=x3) and text bị xoay ngược chiều kim đồng hồ 1 góc 90 độ
                angle_with_horizontal_line = -90
            else:   # guess: vertical rect has p4, p3, p2, p1 (<- y2>y3 and x2=x3)
                angle_with_horizontal_line = 90
        else:
            angle_with_horizontal_line = math.atan2(long_edge[1], long_edge[0]) * 57.296    # arctan(y/x)*57.296
