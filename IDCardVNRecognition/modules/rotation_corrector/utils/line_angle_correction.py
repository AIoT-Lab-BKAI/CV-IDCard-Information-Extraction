...
def rotate_and_crop(idx, img, points, debug=False, rotate=True, extend=True,
    ...
    if extend:
        ...
        ex, ey = 0, 0
    src_pts = box.astype("float32") # [[x1, y1], [x2, y2], [x3, y3], [x4, y4]]
    # width = width + 10
    # height = height + 10
    dst_pts = np.array([
    ...
    # print(M)
    warped = cv2.warpPerspective(img, M, (width + 2 * ex, height + 2 * ey))     # box image

    # print('\n')
    # print('Source point:\n', src_pts)
    # print('Dest point:\n', src_pts)
    # cv2.imwrite(os.path.join('/mnt/e/bkai/ID_Card/IDCardVNRecognition/output_test/test_transform/output_{}.png'.format(idx)), warped)
    # print('\n')
