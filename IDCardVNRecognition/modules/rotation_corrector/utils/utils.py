...
def rotate_image_bbox_angle(img, bboxes, angle):
    def rotate_points(box):  # Rotate each point in 4 points: [x1, y1, x2, y2... x4, y4]
    ...
    if not isinstance(img, np.ndarray):
        img = np.array(img)
    # print('==== Begin rotate_image_bbox_angle')
    shape_ = img.shape  # cccd_1: 775*1246 (h, w, c)
    h_org = shape_[0]  # h = 775
    w_org = shape_[1]  # w = 1246
    # cv2.getRotationMatrix2D: @param: center(x, y), angle, scale
    # angle > 0: ngược chiều kim đồng hồ, trong khi kết quả từ get_mean_horizontal: mean_angle > 0: cùng chiều kim đồng hồ
    Mat_rotation = cv2.getRotationMatrix2D((w_org / 2, h_org / 2), 360 - angle, 1)
    # Mat_rotation = cv2.getRotationMatrix2D((w_org / 2, h_org / 2), angle, 1)

    degr = math.radians(360 - angle)

    rad = math.radians(angle)
    sin = math.sin(rad)
    cos = math.cos(rad)
    # Mở rộng img thành hình chữ nhật lớn chứa cả img
    bound_w = (h_org * abs(sin)) + (w_org * abs(cos))
    bound_h = (h_org * abs(cos)) + (w_org * abs(sin))
    # print((bound_w / 2) - w_org / 2, ((bound_h / 2) - h_org / 2))
    Mat_rotation[0, 2] += ((bound_w / 2) - w_org / 2) - 1
