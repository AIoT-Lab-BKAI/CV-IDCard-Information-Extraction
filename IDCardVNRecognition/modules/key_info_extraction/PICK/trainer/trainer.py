...
class Trainer:
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        # map_location = {'cuda:%d' % 0: 'cuda:%d' % self.app['local_rank']}
        checkpoint = torch.load(resume_path, map_location=self.device)
        self.start_epoch = checkpoint['epoch'] + 1
        self.monitor_best = checkpoint['monitor_best']

        # load architecture params from checkpoint.
        if checkpoint['app']['model_arch'] != self.config['model_arch']:
            self.logger_warning("Warning: Architecture configuration given in app file is different from that of "
                                "checkpoint. This may yield an exception while state_dict is being loaded.")
        self.model.load_state_dict(checkpoint['state_dict'])

        # load optimizer state from checkpoint only when optimizer type is not changed.
        if checkpoint['app']['optimizer']['type'] != self.config['optimizer']['type']:
            self.logger_warning("Warning: Optimizer type given in app file is different from that of checkpoint. "
